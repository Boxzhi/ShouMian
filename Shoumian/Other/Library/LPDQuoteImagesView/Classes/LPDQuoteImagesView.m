...
//            _contentInsets = UIEdgeInsetsMake(10, _margin/2-2, 0, _margin/2+2);
            _contentInsets = UIEdgeInsetsMake(10, _margin, 0, _margin);
        }
...
//    layout.itemSize = CGSizeMake(_itemWH, _itemWH);
    _itemWH = (self.lpd_width - _margin * (_countPerRowInView + 1)) / _countPerRowInView;
    layout.itemSize = CGSizeMake(_itemWH, _itemWH);
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    // 把选中的照片数传出去，代理方法
    [self.navcDelegate selectedPhotosCount:_selectedPhotos.count];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(_selectedPhotos.count<_maxSelectedCount) {
      if (indexPath.row == _selectedPhotos.count) {
          [cell.imageThumbnail setImage:[UIImage imageNamed:@"community_add_photo"]];
